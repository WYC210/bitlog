...
现在除了直接写原始 HTML `<details>`，也支持更顺手的 Markdown 风格写法：

:::details Cloudflare Worker
```ts
export default {
  async fetch() {
    return new Response("hello bitlog");
  }
};
```
:::

如果希望默认展开，可以用 `details-open`：

:::details-open 默认展开的说明
这里适合放“安装步骤”“完整配置”“可选补充说明”这类内容。
:::
...
## 图片标题（figure / figcaption）

<figure>
  <img src="https://picsum.photos/960/420" alt="风景示例">
  <figcaption>这是一张带图注的示例图片。</figcaption>
</figure>
...
<center>这段文字使用 `center` 标签居中。</center>

<p style="text-align:center">这段文字使用 `style="text-align:center"` 居中。</p>
...
> 说明：当前只支持 `left / center / right` 三种对齐值。
...
- <span style="color:purple">紫色文字</span>
- <span style="color:pink">粉色文字</span>
- <span style="color:gray">灰色文字</span>
- <span style="color:teal">青色文字</span>
...
> 说明：当前不是任意颜色都放行，只支持受控颜色名：
> `red` `blue` `green` `orange` `purple` `pink` `gray` `teal`
...
- `style` 目前只会兼容并转换少量受控能力，例如 `text-align` 和 `color`
